...
CHECKPOINT_PATH=checkpoints/gpt2_2.7b/${NNODES}node-${WORLD_SIZE}gpu-dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun
...
mpirun -np $WORLD_SIZE --npernode $GPUS_PER_NODE \
  ...
  --wandb-name "gpt_2.7b_${NNODES}node_dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun-mdx"
