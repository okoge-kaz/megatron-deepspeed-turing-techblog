...
CHECKPOINT_PATH=checkpoints/gpt_6.7b/${NNODES}node-${WORLD_SIZE}gpu-dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun
...
mpirun -np $WORLD_SIZE -machinefile ${PJM_O_NODEINF} \
  ...
  --wandb-name "gpt_6.7b_${NNODES}node_dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun-wisteria"
