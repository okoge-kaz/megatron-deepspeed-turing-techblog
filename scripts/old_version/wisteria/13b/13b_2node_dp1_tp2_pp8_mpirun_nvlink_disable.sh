...
CHECKPOINT_PATH=checkpoints/gpt2_13b/${NNODES}node-${WORLD_SIZE}gpu-dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun-nvlink-disable
...
mpirun -np $WORLD_SIZE -machinefile ${PJM_O_NODEINF} \
  ...
  --wandb-name "gpt2_13b_${NNODES}node_dp${DP_SIZE}-tp${TP_SIZE}-pp${PP_SIZE}-mpirun-wisteria-nvlink-disable"
